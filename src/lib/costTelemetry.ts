  ...
  const sections: CostPanelSection[] = [
    {
      ...
      items: [
        ...
        { label: 'Job geocoding calls', value: formatCount(jobGeocodingCalls ?? usage.geocoding_calls) },
        { label: 'Job autocomplete calls', value: formatCount(usage.autocomplete_calls) },
        { label: 'Job place details calls', value: formatCount(usage.place_details_calls) },
        { label: 'Job input tokens', value: formatCount(usage.input_tokens) },
        { label: 'Job output tokens', value: formatCount(usage.output_tokens) },
      ],
    },
    {
      title: 'Month to date',
      items: [
        { label: 'Estimated monthly total', value: formatCurrency(usage.estimated_monthly_total_usd ?? estimatedMonthlyTotal) },
        { label: 'Month-to-date geocoding usage', value: formatCount(usage.geocoding_calls) },
        { label: 'Month-to-date autocomplete usage', value: formatCount(usage.autocomplete_calls) },
        { label: 'Month-to-date place details usage', value: formatCount(usage.place_details_calls) },
